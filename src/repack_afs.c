...
int main(int argc, char **argv) {
    ...
    for(i=0; i<entries; i++) {
        ...
        // empty entries sometimes occur
		//assert(stas[i].len > 0);
    }
	//bad check when empty entries are present
    //for(i=1; i<entries; i++)
    //    assert(stas[i].pos == ALIGN(stas[i-1].pos + stas[i-1].len));
	...
	/* This probably made sense for pc version
	if(!strcmp(argv[1], "mac.afs")) {
        for(i=0; i<entries; i++) {
	...
	*/
    ...
    int off = ALIGN(entries*8+16);
    for(i=0; i<entries; i++) {
		if (stas[i].pos == 0) { continue; }
        char name[1000];
        sprintf(name, "%s/%s", endir, stbs[i].name);
		FILE *fh = 0;//fopen(name, "r");
        void *p;
        if(fh) {
            //printf("<- %s\n", stbs[i].name);
